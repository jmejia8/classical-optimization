...
function conjugateGradient(f::Function, x0::Vector{Float64}, unidirectionalSearch::Function, ε1::Float64, ε2::Float64, ε3::Float64; debug::Bool=false)
    k = 0
    ∇f(x) = gradient(f, x)
    ...
    x = unidirectionalSearch(f, x_old, s, ε1)
    scatter!(x[1:1], x[2:2])
    ∇fx = ∇f(x)
    fx = f(x)
...
function test()
    ...
    ε1 = ε2 = ε3 = 1e-3
    x0 = [0.0, 0.0]
    conjugateGradient(f, x0, unidirectionalSearch, ε1, ε2, ε2; debug = true)
    #############################################
